...
pos_init_petite = [pm1.x(1), pm1.y(1), pm1.z(1)];
pos_init_grande = [gm1.x(1), gm1.y(1), gm1.z(1)];

% Position extrême = moyenne de toutes les frames (position stable mesurée)
pos_peD = [mean(pe_D.x), mean(pe_D.y), mean(pe_D.z)];
pos_peG = [mean(pe_G.x), mean(pe_G.y), mean(pe_G.z)];
pos_geD = [mean(ge_D.x), mean(ge_D.y), mean(ge_D.z)];
pos_geG = [mean(ge_G.x), mean(ge_G.y), mean(ge_G.z)];
...
% ── Trajectoires (lignes fines en fond) ──────────────────────────────────────
plot3([pm1.x; pm2.x], [pm1.y; pm2.y], [pm1.z; pm2.z], ...
      '-', 'Color', [0.4 0.6 1 0.4], 'LineWidth', 1, 'HandleVisibility','off');
plot3([gm1.x; gm2.x], [gm1.y; gm2.y], [gm1.z; gm2.z], ...
      '-', 'Color', [1 0.4 0.4 0.4], 'LineWidth', 1, 'HandleVisibility','off');
